...
void loop() {
  ...
  // A Complete PID Function that can become P-PD-PI just by setting coefficients
        ...
        void Compute() 
        {
            ...
            integral += (error * SampleTime);  // Calculate integrator participant of the controller output
            if (integral > Imax) { integral = Imax; }
            else if ( integral < Imin) { integral = Imin; } // Anti windup control

            derivative = (Input - lastIn) / SampleTime;   // prevents derivative kick 

            virtualOut = Kp * error + Ki * integral - Kd * derivative;  // Raw Control Output 
